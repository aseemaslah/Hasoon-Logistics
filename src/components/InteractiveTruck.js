  ...
  useEffect(() => {
    ...
    try {
      ...
      const animate = () => {
        ...
        ctx.fillRect(truckX + 205, chassisY + 22, 10, 10);
        ctx.fillRect(truckX + 210, chassisY + 5, 5, 12);

        // Chrome wheel guards
        ctx.fillStyle = "#5C6F8E";
        ctx.beginPath();
        ctx.arc(truckX + 35, chassisY + 32, 18, Math.PI, 0);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(truckX + 68, chassisY + 32, 18, Math.PI, 0);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(truckX + 188, chassisY + 32, 18, Math.PI, 0);
        ctx.stroke();
      ...
      console.error("InteractiveTruck canvas initialization error:", err);
      // Fallback display on crash
      const width = canvas.width || 300;
      const height = canvas.height || 200;
      ctx.fillStyle = "#0A101D"; // Dark cyber background
      ctx.fillRect(0, 0, width, height);
      ctx.fillStyle = "#C6A75E";
      ctx.font = "bold 14px 'Inter', sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("HASOON LOGISTICS OVERLAND FLEET", width / 2, height / 2 - 10);
      ctx.fillStyle = "#94A3B8";
      ctx.font = "10px sans-serif";
      ctx.fillText("Securing GCC cross-border road transport corridors.", width / 2, height / 2 + 15);
      cleanupFn = () => {};
    }
